...
import math
import numpy as np
from rfdetr.util.box_intersection_2d import oriented_box_intersection_2d
from rfdetr.util.min_enclosing_box import smallest_bounding_box
...
def cal_iou(box1:torch.Tensor, box2:torch.Tensor):
    ...
    if corners1.shape[0] == 0:
        breakpoint()
    if torch.equal(corners1,corners2):
        inter_area = area1
    else:
...
def cal_giou(box1:torch.Tensor, box2:torch.Tensor, enclosing_type:str="smallest"):
    ...
    # if math.isnan(giou): 
    #print(f"giou:{giou}, area_c:{area_c}, w:{w}, h:{h}, corners1:{corners1},corners2:{corners2}")
    #print(iou)
    return giou, iou
    # return iou, iou
