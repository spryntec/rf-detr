...
'''
import torch
from rfdetr.util.cuda_op.cuda_ext import sort_v
EPSILON = 1e-8
...
def build_vertices(corners1: torch.Tensor, corners2: torch.Tensor, 
    ...
    Returns:
        ...
        mask (torch.Tensor): (B, N, 24)
    """
    B, N = corners1.shape[:2]


    # # Check if inters has zero elements ? avoid view error
    # if inters.numel() == 0:
    #     breakpoint()
    #     # No intersection: only corners1 and corners2 contribute
    #     vertices = torch.cat([corners1, corners2], dim=2)  # (B, N, 8, 2)
    #     mask = torch.cat([c1_in_2, c2_in_1], dim=2)         # (B, N, 8)

    #     # Pad to (B, N, 24, 2) and (B, N, 24)
    #     pad_vertices = torch.zeros((B, N, 16, 2), dtype=corners1.dtype, device=corners1.device)
    #     pad_mask = torch.zeros((B, N, 16), dtype=torch.bool, device=corners1.device)

    #     vertices = torch.cat([vertices, pad_vertices], dim=2)  # (B, N, 24, 2)
    #     mask = torch.cat([mask, pad_mask], dim=2)              # (B, N, 24)
    # else:
    #     # Standard case
    inters_reshape = inters.view(B, N, inters.shape[2] * inters.shape[3] , 2)
    mask_inter_reshape = mask_inter.view(B, N, mask_inter.shape[2] * mask_inter.shape[3])
    vertices = torch.cat([corners1, corners2, inters_reshape], dim=2)  # (B, N, 24, 2)
    mask = torch.cat([c1_in_2, c2_in_1, mask_inter_reshape], dim=2)       # (B, N, 24)
