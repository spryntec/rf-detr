...
import torch, os
import math
from rfdetr.util.oriented_iou_loss import cal_giou, cal_iou
...
#rajouter clam .clamp(min=0.0) dans la fonction ci dessous
...
def generalized_box_iou(boxes1, boxes2):
    ...
    """
    # Check if the assertion condition fails
    if not (boxes1[:, 2:] >= boxes1[:, :2]).all():
        print("Invalid boxes1 detected (x1 < x0 or y1 < y0):")
        for i in range(boxes1.shape[0]):
            top_left = boxes1[i, :2]  # [x0, y0]
            bottom_right = boxes1[i, 2:]  # [x1, y1]
            if not (bottom_right >= top_left).all():
                print(f"Box {i}: top-left = {top_left.tolist()}, bottom-right = {bottom_right.tolist()}")

    # Assert that x1 >= x0 and y1 >= y0 for all boxes in boxes1
    assert (boxes1[:, 2:] >= boxes1[:, :2]).all(), "Invalid boxes1: some x1 < x0 or y1 < y0"
    assert (boxes2[:, 2:] >= boxes2[:, :2]).all(), "Invalid boxes2: some x1 < x0 or y1 < y0"
...
def generalized_box_iou(boxes1, boxes2):
    ...
    assert (boxes1[:, 2:] >= boxes1[:, :2]).all(), "Invalid boxes1: some x1 < x0 or y1 < y0"
    assert (boxes2[:, 2:] >= boxes2[:, :2]).all(),"Invalid boxes1: some x1 < x0 or y1 < y0"
    # except:
    #     import ipdb; ipdb.set_trace()
    iou, union = box_iou(boxes1, boxes2)
